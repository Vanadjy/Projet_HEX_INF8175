...
class MyPlayer(PlayerHex):
    ...
    def heuristic_parameters(self, state: GameState, nb_maillons: int, pcc):
        ...
        '''if pcc >= 11: # Si le chemin qu'il reste à parcourir est de moins de 4 tuiles : mettre la priorité dessus
            ...
            return [1.0, 0.5, 0.4, 0.5]'''
        weight = (state.step+1)/(65)
        return [weight, 0.3*(1-weight), 1.2*(1 - weight), (1 - weight)]
    ...
    def local_pressure(self, state: GameState):
        current_rep = state.get_rep()
        env = current_rep.get_env()
        adv_pressure = 0
        # On regarde sur tous les coups jouables
        for empty_i, empty_j in current_rep.get_empty():
            neighbors = state.get_neighbours(empty_i, empty_j)
            # On compte le nombre de voisins de la couleur adverse
            for n_type, neigh in neighbors.values():
                if n_type == state.next_player.piece_type:
                    adv_pressure += 1
        return adv_pressure
    ...
    def shortest_path(self, state: GameState, player_color):
        """
        ...
        while len(pq) != 0:
            ...
            for n_type, (ni, nj) in state.rep.get_neighbours(i, j).values():
                ...
                if new_dist is not None:
                    ...
                    dist[ni, nj] = np.inf
                    heapq.heappush(pq, (np.inf, (ni, nj), (i, j)))
    ...
    def heuristic_shortest_path(self, state: GameState):
        ...
        my_shortest_path = self.shortest_path(state, self.piece_type)
        opp_shortest_path = self.shortest_path(state, state.next_player.get_piece_type())
        #opponent_shortest_path = self.shortest_path(state_opponent)
        return my_shortest_path, opp_shortest_path # 13 est la longueur du chemin en ligne droite 
    ...
    def heuristic_block(self, state):
        block = 0
        current_rep = state.get_rep()
        if self.piece_type == 'R':
            nb_blue_cases, blue_cases = current_rep.get_pieces_player(state.players[1])
    ...
    def master_heuristic(self, state:GameState):
        ...
        nb_maillons = self.heuristic_maillon(state)
        pcc, opp_pcc = self.heuristic_shortest_path(state)
        params = self.heuristic_parameters(state, nb_maillons, pcc)
        #opp_influence = self.heuristic_opponent_influence(state)
        adj_pressure = self.local_pressure(state)
        
        '''print("VALUES OF HEURISTICS - PCC - NB_M - OPP_INFL")
        print(params[0]*pcc)
        print(params[1]*nb_maillons)
        print(params[3]*get_centre)'''
        #return - params[0]*pcc/14 + params[2]*opp_pcc/14 + params[1]*nb_maillons/14*14 + params[3]*get_centre #+ 0.4*params[2]*opp_influence
        return 1.5*(pcc) + adj_pressure + 2*nb_maillons
